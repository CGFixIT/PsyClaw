...
- **Residual — RESOLVED 2026-09-04, it did flag.** The prediction above came true: pip-audit began reporting [PYSEC-2026-3740](https://osv.dev/vulnerability/PYSEC-2026-3740) (= CVE-2026-81726) against the 3.10.3 pin, first red on the scheduled `main` run of 2026-09-03 ([33722097917](https://github.com/cgfixit/CyClaw/actions/runs/33722097917)). This is the re-triage that note called for, not a pre-ignore.
  - **Advisory:** path traversal in model-artifact APIs that bypass pathsec by using raw file operations on caller-controlled paths. Named affected APIs: `TransitionParser`, `AveragedPerceptron`, `PerceptronTagger`, and `maxent` parameter APIs. CVSS v4.0 4.0.
  - **Why accepted:** none of those four is imported anywhere in this repo — a grep across every `*.py` returns only comments stating CyClaw avoids them. The only nltk usage remains `from nltk.stem import PorterStemmer` (`retrieval/stemmer.py:15,26`), a pure string transform that opens no files and takes no path argument. The vulnerable code paths are unreachable in this deployment, the same shape of argument as the chromadb acceptance above.
  - **No fix to take:** `nltk==3.10.3` is the newest release on PyPI and pip-audit reports an empty Fix Versions column. There is no version to bump to — unlike the 3.10.2 cluster, which the #1256 bump did close.
  - **Sources disagree, recorded rather than smoothed over:** GHSA-8mgp-746c-j5xp gives affected `<= 3.10.3` / patched `None`; the PYSEC record appears to carry `fixed: 3.10.3` while its own `details` string says “through 3.10.3”. `osv.dev` and `advisories.gitlab.com` were egress-blocked from the session that triaged this, so the raw advisory was not read first-hand. pip-audit's live behaviour — it flags 3.10.3 and lists no fix — is the operative fact either way.
  - **Scope of the ignore:** `pip-audit.yml` only, keyed on `PYSEC-2026-3740` (the exact ID the scanner reports), NOT the `CVE-2026-81726` alias. `.trivyignore` and `.osv-scanner.toml` get no entry — neither scanner flags it, and a dead entry reads as a live risk-acceptance (`.osv-scanner.toml`'s own hygiene rule). The extras loop is untouched: nltk is not an extra.
  - **Delete this acceptance if** the advisory data is corrected so 3.10.3 is no longer reported, or a patched release ships. Verify with `pip-audit -r requirements.txt -r requirements-test.txt` minus this ignore.
- **Guardrails (extends the row above):** any change importing `TransitionParser`, `AveragedPerceptron`, `PerceptronTagger`, or the `maxent` parameter APIs MUST be treated as a security regression and re-open this assessment.
