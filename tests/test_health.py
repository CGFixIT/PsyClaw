...
class TestCheckAll:
    ...
    def test_offline_ollama_configured_model_present_is_healthy(self, tmp_path, monkeypatch):
        ...
        cfg_path = _write_cfg(tmp_path, mode="offline", local_model="qwen3.8:27b")
        monkeypatch.setattr(
            health, "_http_get",
            lambda url, **kw: _ModelsResp(["qwen3.8:27b", "nomic-embed-text"]),
        )
        statuses = health.check_all(cfg_path)
    ...
    def test_offline_ollama_missing_model_pin_reports_unhealthy(self, tmp_path, monkeypatch):
        ...
        assert "qwen3.8:27b" in ollama.error
