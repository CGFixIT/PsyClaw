...
def test_run_threads_planner_limits_into_the_local_client(cfg_path, checks_file, monkeypatch, capsys):
    ...
    iteration 1 with no retry. The CLI must pass the validated timeout to the
    client and the completion budget to its invoke call.
    """
    captured_limits = []

    def fake_invoke(self, *, system_prompt, user_prompt, max_tokens=2048, temperature=0.0,
                     config_path="config.yaml", cfg=None):
        captured_limits.append((self._client.timeout.read, max_tokens))
    ...
    assert _run_start(cfg_path, checks_file) == EXIT_OK
    assert captured_limits == [(723.0, 3072)]
