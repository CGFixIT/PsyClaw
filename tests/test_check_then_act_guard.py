...
def test_authn_manager_has_no_unguarded_select_then_insert():
    ...
    This shipped with a temporary accepted set carrying create_user and
    create_device_token, because the guard landed while their fix (#1216) was
    still an unmerged branch. #1216 is on main now, so the guard runs live:
    any offender fails, grandfathered names included.
    """
    ...
    assert not offenders, (
        ...
        f"{sorted(offenders)} -- the pre-check SELECT means a uniqueness "
        "constraint exists, so wrap the INSERT in try/except using "
        "_is_unique_violation (see bootstrap_if_empty for the reference "
        "shape), or fold the conflict into the SQL"
    )
