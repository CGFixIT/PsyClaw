...
| Runtime envelope | `reasoning_effort: none`; `OLLAMA_CONTEXT_LENGTH=16384`; one loaded model and one parallel slot. This is the deliberately bounded local `/query` setup, not a claim about the maximum context the hardware can hold. |
...
4. **Is `num_ctx` set high enough on the Ollama side for the current `/query` envelope?** `config.yaml` reserves `max_tokens` (4096) up front, and the RAG request can supply `max_context_tokens` (8000). The documented floor is therefore 8000 + 4096 + ~1500 = 13,596; the shipped `OLLAMA_CONTEXT_LENGTH=16384` leaves headroom. `num_ctx` is a server-side setting, not a CyClaw per-request control, so confirm it on the operator's machine. The failure symptom is a silent stall at "0% processing," not necessarily an error.
