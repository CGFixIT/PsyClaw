...
**Superseded note (2026-08-15):** `config.yaml`'s `models.local_llm.model` (and the paired `agentic.deepagent_github.model` / `guardrails.model` knobs) moved from `qwen3.6:27b` to `qwen3.8:27b-mlx`. Everything below — the model-identity findings, the GGUF/llama.cpp friction analysis, and every citation in the "Qwen3.6 verification" Sources section — was researched against `qwen3.6:27b` specifically and does **not** carry over to `qwen3.8:27b-mlx`. Treat this note as a historical record of the `qwen3.6:27b` investigation, not as current guidance; a fresh research pass against `qwen3.8:27b-mlx` is a prerequisite before trusting any of its conclusions for the model CyClaw now ships.

**Tag existence, checked 2026-08-15 (the one claim that was verified for the new default):** `qwen3.8:27b-mlx` is a real, published Ollama library tag — `https://ollama.com/library/qwen3.8` lists `qwen3.8:latest`, `qwen3.8:27b`, and `qwen3.8:27b-mlx` among 12 tags, with weights released 2026-08-14 under Apache 2.0 (dense 27B; default build an ~18GB Q4_K_M). This closes only the "does the tag resolve" question that the *first* Phase-2 open question below asks about the old tag. Everything else in this note — the GGUF/llama.cpp architecture-support analysis, the `unknown model architecture` friction pattern, the vendored-build lag — was researched against 3.6 and has NOT been re-verified for 3.8. The load test on the operator's own machine (`ollama pull qwen3.8:27b-mlx` and load it) is still the check that matters.
