...
| 6 | Tier 2 — agentic/db — **PARTIAL 2026-08-03** | `langgraph==1.2.9` (:43), `langchain==1.3.14` (:81), `langchain-openai==1.3.5` (:82) — bumped across all four install surfaces. `psycopg`/`psycopg-binary==3.2.13`, `pgvector==0.4.2` **not bumped**: their plan-listed verification needs a live Postgres/pgvector service (the `postgres-backend` CI job's own target), not runnable standalone | The langgraph/langchain/langchain-openai bumps ran the full pytest gate locally; psycopg/pgvector stay queued until their bump can be verified against the live service |
| 7 | Tier 3 — web/core — **DONE 2026-08-03** | `fastapi==0.139.2` (:33), `uvicorn==0.51.0` (:37), `langchain-core==1.5.0` (:44) | Touches the `gate.py` request path. FastAPI 0.138→0.139 changelog checked first (additive-only: `app.frontend()` feature + translations, no breaking changes); full pytest gate green locally. `environment.yml`'s deliberate `fastapi=0.115.9` conda exception untouched |
...
| 9 | `httpx2` migration | `httpx==0.28.1` (:39) | Starlette's `TestClient` steers onto `httpx2`; `pyproject.toml` currently filters the deprecation warning by exact message. No runtime impact today — owed before a future Starlette major hard-cuts over. Source: `docs/analysis/TESTCLIENT_HTTPX_DEPRECATION.md` |
