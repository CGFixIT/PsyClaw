...
**Current wiring status: WIRED as of 2026-09-04.** It is registered as the second entry in `.claude/settings.json`'s `hooks.SessionStart` array, alongside the `python-coding-agent` context loader, as `bash .claude/hooks/session-start-sync-check.sh`. *(Historical: until that date the script sat on disk unregistered — `SessionStart` held only the persona loader.)*
