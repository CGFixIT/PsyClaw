...
`docs/POSTGRES_BACKEND.md`, `docs/analysis/TESTCLIENT_HTTPX_DEPRECATION.md`,
...
Proposes a `SessionStart` hook, `.claude/hooks/session-start-sync-check.sh`, to close a recurring failure mode: local `main` drifting from `origin/main` (wrong-identity commits flagged "Unverified," ahead/behind divergence after remote merges, `git reset --hard` reached for as an ad-hoc — and destructive — fix). The hook is deliberately advisory only: on session start it (1) pins commit identity repo-locally to `cyclaw-agent@users.noreply.github.com`/`CyClaw Agent` — the driver-agnostic default from `utils/agent_identity.py`, not an Anthropic identity, because the loop is often a local model or another agent — (2) fetches the default branch read-only and reports ahead/behind counts, (3) prints reconciliation guidance (ff-only when safe, review `git log origin/main..HEAD` before discarding) without ever running reset/rebase/push/delete itself, and always exits 0 so it can never block a session. The doc also states the underlying operating conventions independent of the hook: treat local `main` as read-mostly, `git fetch && git merge --ff-only` after a remote merge rather than reset, treat `git reset --hard` as a last resort with a preservation branch first, and rely on pinned identity for verifiable commits.

**Current wiring status: WIRED as of 2026-09-04.** It is registered as the second entry in `.claude/settings.json`'s `hooks.SessionStart` array, alongside the `python-coding-agent` context loader, as `bash .claude/hooks/session-start-sync-check.sh` — the exact form this doc's own wiring section specifies. *(Historical: from this file's original writing until 2026-09-04, the script sat on disk unregistered — `settings.json`'s `hooks.SessionStart` held only the `python-coding-agent` loader — and shipping the registration was left to a human decision. That decision was made in session on 2026-09-04, not by a PR turning it on silently.)*
