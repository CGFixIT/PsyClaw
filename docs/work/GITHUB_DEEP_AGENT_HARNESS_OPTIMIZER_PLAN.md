...
| `agentic/registry.py` | Governed skills registry. | `propose_skill` never writes; `apply_skill` requires the `agentic.enabled` master switch, write mode, `writes_enabled`, human reason, injection scan, atomic write, SHA-256 history, audit. | Reuse governance pattern for accepted optimizer improvements. | Do not create a bypass write path for skills or prompts. |
