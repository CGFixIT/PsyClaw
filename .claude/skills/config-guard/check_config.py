...
def run_checks(cfg: dict[str, Any]) -> None:
    ...
    print("C13 security.api_key_optional vs. LAN-reachable host list")
    ...
    if not api_key_optional:
        ...
        host = _dig(cfg, "api", "host")
        allowed_hosts = _dig(cfg, "security", "allowed_hosts")
        lan_hosts = [
            h for h in (allowed_hosts or []) if isinstance(h, str) and h not in _LOOPBACK_HOSTS
        ]
        if host not in _LOOPBACK_HOSTS or lan_hosts:
            reason = f"api.host={host!r}" if host not in _LOOPBACK_HOSTS else f"allowed_hosts includes {lan_hosts}"
            warn("C13", f"security.api_key_optional=true with a LAN-reachable posture ({reason}) — "
                        "every CYCLAW_API_KEY-gated route (gate.py's soul/ops/memory/audit endpoints "
                        "and the harness's guarded routes, including agent run/push/publish) is "
                        "unauthenticated for any device that can reach that host/port, not just localhost")
        else:
            ok("C13", "security.api_key_optional=true but posture stays loopback-only "
                      "(api.host and allowed_hosts carry no LAN entries)")
