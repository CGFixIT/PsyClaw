...
Run a full optimization sweep of CyClaw main and open draft PRs only for findings that earn a focused chunk (guideline count, not a quota). $ARGUMENTS
...
> `/CyClaw-Sandbox` skill -- its Quick Mode, or `/run` (note the CyClaw
