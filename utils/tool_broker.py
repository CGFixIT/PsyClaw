...
Lives in ``utils/`` so harness (and later callers) can use it without
importing ``guardrails`` (I6: harness must not import sibling OOB packages).
...
``guardrails.tool_broker`` re-exports this module. ``guardrails.broker`` is
the NVIDIA ``check()`` wrap around generation — not a tool broker.
